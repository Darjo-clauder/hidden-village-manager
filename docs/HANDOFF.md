...
> | 2 | **Visual overhaul** — `docs/VISUAL_OVERHAUL.md`. Done: motion (§3.7), type (§3.1), Roster / Transfers / Scouting / Academy on `hv-table`/`hv-attr`/`hv-inspector` (§3.3–3.4). **Next, in order: (a) Staff and the Exam leaders table (the last inline `tblHeaderHtml` caller, `exam.js:1324`) onto `hv-table`; (b) the remaining §3.4 components — `.hv-meter` for the hand-built bars, `.hv-kpi` for the dashboard tiles, `.hv-card` to collapse the eight card near-duplicates; (c) charts (§3.5, `charts.js`, radar first).** Keep the pattern: select without a full rebuild, preserve inspector scroll across `upUI()`, `hvKeyNav` for Esc/↑↓ | weeks | nothing |
