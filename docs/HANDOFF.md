...
> | 2 | **Visual overhaul** — `docs/VISUAL_OVERHAUL.md`. Done: motion (§3.7), type (§3.1), Roster / Transfers / Scouting / Academy / Staff on `hv-table` + `hv-inspector` (§3.3), `hv-meter`/`hv-card`/`hv-kpi` (§3.4), radar + `charts.js` (§3.5). **Next, in order: (a) the remaining 13 hand-built bars outside the dossier onto `hvMeterHtml` (`grep -c 'height:[2-6]px[^"]*"><div style="[^"]*width:'` finds them — squads, kagedev, dashboard, world, village, upgrades, transfers, staff, safehouses, finances, economy); (b) line charts from `G.analyticsHistory` (monthly snapshots already exist) on the dashboard — treasury, morale, reputation with the league average in `--viz-6`; (c) §3.3 shell: the context bar and the 6-group nav; (d) §3.6 icons — replace emoji.** Keep the pattern: select without a full rebuild, preserve inspector scroll across `upUI()`, `hvKeyNav` for Esc/↑↓, card body becomes inspector body verbatim | weeks | nothing |
