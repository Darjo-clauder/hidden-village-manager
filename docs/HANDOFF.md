...
**Last updated:** 2026-09-06 · **HEAD:** `cf3159e` (committed + pushed, mirror ff'd) · **Branch:** `master` · **Tests:** 1360 passing / 103 files

---

> ## ▶ START HERE
>
> **The build is healthy and nothing is broken.** Tests green, all three clones in sync, **desktop installer rebuilt from `cf3159e`** (`src-tauri/target/release/bundle/nsis/`). There is no fire to put out. *(Rebuilding does not reinstall: the copy under `AppData` is still whatever was last run through the installer — run the fresh setup.exe to look at the real thing; see pinned item 3.)*
>
> **Tyler's call on 2026-09-05: the visual overhaul is top priority** — the game felt clunky and unresponsive. Thirteen commits landed against `docs/VISUAL_OVERHAUL.md`: `25cb4b3` the *feel* pass (press/hover/motion — measured first: a click showed nothing for 20–110ms, there was no `:active` rule at all), `3d1dad0` *type* (IBM Plex Sans/Mono + Spectral bundled via @fontsource, every font tokenised, ramp raised to the 11px floor), and `16f46d4` + `21320e1` **Roster as the first panel on the `hv-*` library** — `.hv-table` with value-tinted attributes, the dossier in a sticky `.hv-inspector` beside the list (a drawer below 1360px), filter bar, ↑/↓ selection; `oDos` is now a thin wrapper over `_dossierHtml(s)`. Then the same table + inspector shape for **Transfers** (`ed56e55`, the market), **Scouting** (`c68c31a`, one prospects table with facets instead of three card grids) and **Academy** (`0161b5d`, the heaviest panel: 1,818 nodes → 558, 80ms → 28ms). Then **Staff** as a slot board + the Exam leaders table (`575680c`), the **components** pass — `hv-meter` replacing every dossier bar, `hv-card` as the one surface the eight card classes alias to with the brief's lit top edge, `hv-kpi` (`e96f1ad`) — and **charts** (`5efc734`): `client/js/charts.js` with the attribute **radar** in every dossier and prospect file, village average as the dashed comparison, `--viz-*` palette defined. Then `416503f` every remaining hand-built bar in the client onto `hvMeterHtml` (none left — `grep 'height:[2-6]px[^"]*"><div style="[^"]*width:'` returns nothing); `29fea09` **dashboard Trends** (treasury/morale/reputation line charts from `G.analyticsHistory`, own series only — the league is not snapshotted) and a real bug: Finances' reputation sparkline read `s.rep` from snapshots that store `reputation`, flat zero since it shipped; `cf3159e` **shell** — nine nav sections → the brief's six (Village · Squad · Matches · World · Academy · Legacy, buttons moved not renamed, duplicate World Map entry dropped) and a **Next** readout in the top bar (nearest exam/summit/intake). Library helpers in `uikit.js`: `hvInspectorHeadHtml`, `hvKeyNav`, `hvMeterHtml`, `.hv-split`. All DOM-verified only — **nobody has looked at any of it yet**; open `http://localhost:5173` after `npm run dev` (port 3000 serves the stale `dist`). The playtest stance still stands underneath: twelve systems have shipped unplayed.
>
...
> | 2 | **Visual overhaul** — `docs/VISUAL_OVERHAUL.md`. Done: motion (§3.7), type (§3.1), Roster / Transfers / Scouting / Academy / Staff on `hv-table` + `hv-inspector` (§3.3), `hv-meter`/`hv-card`/`hv-kpi` (§3.4), radar + `charts.js` (§3.5). Also done: all bars on `hv-meter`, dashboard Trends, the six-group nav + Next readout. **What is left of the brief: (a) §3.6 icons — replace the emoji with a single-weight line set (this is art direction; get Tyler's eyes on everything above before drawing 30 icons blind); (b) snapshot the league in `adv.js` next to `analyticsHistory` so Trends can draw the `--viz-6` comparison line the brief asks for; (c) the panels still on cards that would read better as tables — squads (fine as cards), missions board, meetings; (d) the inline-style count: `grep -o 'style="' client/js/panels/*.js | wc -l` was 3,072 at the brief and is 2,812 at `cf3159e` — the honest progress metric; the table conversions removed the layouts, not yet the inline colours inside cells.** Keep the pattern: select without a full rebuild, preserve inspector scroll across `upUI()`, `hvKeyNav` for Esc/↑↓, card body becomes inspector body verbatim | weeks | nothing |
...
>
> - **S2 depth-chart pressure (`b4cad87`)** — `shared/utils/depthPressure.js`. Reuses the existing per-shinobi `workload` signal as one opportunity meter: **Rusty** (<15, bench rot) and **Overworked** (>75) both cut the monthly stat-growth roll; only the middle band develops at full rate. Wired into adv.js growth; surfaced as a live band label on depth-chart tiers, starter rows, and the hover card. 6 tests.
> - **M3 squad cadence (`e233edb`)** — `shared/utils/squadCadence.js`. Squads deployed 3+ consecutive months take a success penalty (caps −15%) plus extra cohesion loss on a defeat; squads idle 2+ months bleed cohesion (caps 15/mo). **Deployment is read live off `G.aM` each tick, not set at resolution** — the first cut used a resolution-only flag, which wrongly counted mid-flight multi-month missions as *idle*. Squad card shows ⚠ grind / 💤 idle. 11 tests. Verified live: 3 straight months → "−3% success penalty"; 2 idle → cohesion 50→48.
> - **D2 leverage plays (`19c30d5`)** — `shared/utils/leverage.js` + a LEVERAGE tab in Intel. Three plays that each attack a *specific* field the world model already tracked as read-only flavour: **Blackmail** (needs soured `kagePersonalRel`; success sets `v.demandsSuppressedUntil`, honoured in kage.js so their tribute demands genuinely stop), **Discredit Their Ace** (needs `v.aces`; dents strength), **Poison the Bloc** (needs `G.summitBlocOffer`; clears it). **Recon is ammunition** — every play requires and consumes a current intel report, so scouting finally pays off downstream. Failure always exposes you; heat feeds back into future odds. 14 tests.
> - **E1 budget allocation (`d76bec8`)** — `shared/utils/budgetRamp.js`. The three sliders were independent and only *warned* when they didn't sum to 100 — **nothing enforced it, so all-three-at-100% granted max training AND war prep AND infra simultaneously** (real exploit, now closed by normalising to a zero-sum 100). Allocation is now a TARGET; the sim reads an EFFECTIVE split that walks ~6 pts/month (full swing ≈ two quarters), so funding is a bet placed early rather than an instant readout. `allocationEffects()` is the single source of truth shared by panel preview and sim. 16 tests. Verified live: 33→43→54→64→76→88→100 then holds.
> - **Visual overhaul (`86313c2` tokens, `462aa07` migration)** — direction chosen by user: **"ink & parchment"**. Root cause of "too generic" was that the app ran **two half-migrated palettes at once** (cool-slate tokens + a stray warm-brown set) across ~2,800 raw hex literals. All of it now resolves to tokens in `client/style.css` — the whole game is re-themable from that one block. Added the character layer: CSS-only paper-tooth grain + vignette, ink-wash sidebar, lacquered top bar, sticky panel headers with a gold brush-stroke rule, seal-red disc behind the title mark, plus shared classes (`.sect/.stat/.chip/.tbl-dense`) and a named type scale replacing ad-hoc 6–22px sizing.
>
> **Three real bugs found during the visual pass** (all pre-existing): (1) `_hexAlpha` in ui.js parsed the literal string `"var(--gold)"` whenever no nation was selected → `--accent-bg/-border/-sb` computed to `rgba(NaN,…)`, **silently killing every accent tint in the game's default state**; now validated with a real hex fallback (nation theming re-verified: Ember red-orange, Tide blue, no NaN). (2) Academy prospect cards rendered `"undefined · Age 14"` when a prospect had no spec — the sibling hover line already guarded it. (3) Prospects are unranked, so `RANKS[p.ri]` printed a literal `"undefined"` rank badge → now "Prospect".
>
> **Verification method (screenshots unavailable):** the Browser pane never composites in this environment, so `computer{screenshot}` times out — verified instead by driving the game through `window.__G` (temp hook, reverted before every commit) and asserting on computed styles + DOM. Final sweep across **all 28 panels: zero render errors, zero unresolved CSS variables, zero undefined/NaN leaking into the DOM**, no console errors.

> **Bracket board VERIFIED (2026-07-10, no code change):** drove a full Adept Exam via the quick-form path (schEx → quickFormExamCells → startEx → runRound ×4 → watchExam) with the temp `window.__G` QA hook (reverted). Confirmed live: Proving Forest arena, 5v10 bracket field, `⇄ Line change` ticker at period boundaries with rotated line offsets, and a full-match stats sheet crediting all five line members (№1–№5) — rotation genuinely distributes play. ELIMINATED · Reached Final rendered correctly. Zero console errors. QA gotcha for next time: the landing "Continue ▸" click can silently fail — always probe `Object.keys(window.__G).length` (when hooked) rather than trusting `btn-end-turn` existence (the shell renders it even on the setup screen).

> **Bug-clean pass (`96b7908`):** five viewer bugs fixed — (1) double-fired battle call re-ran the scheduler and double-counted state (latched via `ov.__callHandled`); (2) call countdown bar ignored playback speed; (3) same-tick clock items fired in insertion order (period's final strike could land after its beat text — due items now time-sorted); (4) mid-match skip left a fragment stats sheet (`_finishInstant` now eagerly completes unstaged periods into the event log — verified live); (5) beat re-reveals double-drained display stamina / double-counted KOs (idempotent via `ov.__revealed`). Design note: watch-vs-skip sheets may differ — live runs evaluate situations (tempo changes), skip uses base tempo.

> **Match engine COMPLETE — steps 3-7 (`b753818`):** `shared/constants/tactics.js` (team knobs auto-mapped from village identities; situational triggers late-push/protect/man-down/power-phase/redline; 7 tests) + `matchEngine.buildPeriod` (lazy per-period seeded builds so situations reshape the play via effective TEMPO) + `statsFrom` event-log aggregation. liveBattle: ⚑ situation announcements at period boundaries (verified live: "Protecting the lead", "Power phase"), away side plays its identity's tactic row, **L/P/Z/S overlay chips** (lanes w/ blocked-lane detection, pressure gradients, boosted zone read, shape lines — one at a time, verified), **post-match stats sheet** (possession/strikes/pass%/per-actor, verified "85% · 5(1)–0(1) · ✦5"), MOTM tie-break now sim contribution then stamina, bracket boards rotate lines each period (⇄ — code-complete, needs a played exam to drive visually). **Fixed a real scheduler bug**: callbacks that `_sched` more work had additions clobbered by the queue-filter overwrite; due items now split out before firing. Zero console errors, headless-verified.

> **MATCH ENGINE OVERHAUL underway — blueprint + steps 1-2 shipped.** `docs/MATCH_ENGINE_BLUEPRINT.md` (commit `8265b79`) is the full design: possession-phase sim under the fixed beat results (outcome governor), token anatomy, overlays, 3-tier tactics, event log → ticker/stats, 7-step build order. **Steps 1+2 live (`dc3a0ec`):** pure seeded `shared/utils/matchEngine.js` (buildMatchScript honours every beat — 200-seed governor sweep in tests; 5-band hex zones; tickerLine), pitchView token anatomy (role badge, element ring, stamina arc, facing wedge, possession pip), separation steering (no more blob), phaseEvent choreography (carrier + objective, pass lines, ✦/▣/⚡/✕ markers, zone tint), liveBattle period/phase scheduler + speed-filtered commentary ticker. **Hidden-tab rAF fallback** added (66ms interval when document.hidden) — keeps board+text in sync when backgrounded AND enables headless QA (this session's Browser pane never composited; verified via pixel sampling + ticker/behaviour, zero console errors). **Next steps (3-7):** decisions+governor at sim level, overlays (L/P/Z/S chips), team tactics + identity auto-map + situations, stats sheet, multi-squad bracket boards.
...
> **Match view BIG UPDATE — 6 features (`9a2a881`):** (1) **Auto-resolve** — settle a match instantly, no animation; same closures apply so result == watching. (2) **Match preferences** (⚙ popover, persisted on `G.matchPrefs`, guarded by `resolveMatchPrefs`) — default tactic + battle call used by auto-resolve; skip/auto share `_finishInstant`. (3) **Objective token** — a contested scroll dragged toward whoever's winning. (4) **Role-tinted circles + legend** (`ROLE_TINT` exported from pitchView) — home circles coloured by squad role, opposition = arena accent. (5) **Crowd reactions** — stands brighten/jostle + floating ROAAR/groan/OOF/CHAMPIONS. (6) **Player of the Match** — top grade tie-broken by stamina discipline, badge in outcome. Pure additions in matchSim.js (`simulateFinalStamina`, `resolveMatchPrefs`, `playerOfMatch`) +5 tests. All browser-verified on a driven game (auto-resolve instant w/ default Commit + condition settled; MOTM "Genta Mori — A commanding flex display"; objective/legend/spotlight on board). Zero console errors.
...
> **Match condition sim (`4ebb6a7`):** the match window is now a touchline. Members enter with stamina from real condition (`staminaStart`: chakra − carried workload), each beat drains by role × live tactic (Press/Balanced/Conserve dial in the viewer) × result; unit comp multiplies (balanced trio +8 start, medic regen on wins, support −10% drain, double vanguard +15% burn — `unitCompRead`). Finish verdict (`finishEffects`: Fresh finish −5 fatigue +2 morale / Hard-worked / Ran ragged +9 fatigue −1 morale) applies REAL deltas once via `rep.applyCondition` closure (live-only, same pattern as battle calls; `rep.matchStamina` is serializable data, closure drops on save). Pure sim `shared/utils/matchSim.js` (+9 tests). Stamina bars under player circles, condition readout + comp tags in `#bv-tactics`, stamina in click-inspect. Replay resets legs, result stays locked. **Browser-verified end-to-end on a fresh driven game** (fresh-game bootstrap works via `beginGame()` — the earlier "Begin didn't start" flag was QA automation clicking a scenario card, not a real bug). Screenshot tooling timed out this session (page healthy, all DOM assertions passed); grab a visual next session if wanted.

> **Hex stadium + interactive playback (`8265dd2`):** the match view is now a hexagon stadium (crowd stands ring the hex arena; terrain/props/pitch-grammar clipped inside; players radially bounded) with real interaction — hover for nameplates, click a circle to inspect (role + match grade strip under the pitch, glow ring on selection), and pitch controls: ⏸ pause, 1×/2×/4× speed, ↻ replay-from-kickoff. Enabled by replacing the setTimeout reveal chain in `liveBattle.js` with a pausable speed-scalable match clock (`_startClock`/`_sched`); replay locks any pending micro-call as Disengage first (outcome/rewards immutable). Browser-verified on the driven save (click-inspect, 4× replay, pause) — zero console errors.

> **Animated match engine view (2026-07-08, `429c974`):** FM24/FHM-style 2D pitch in the live battle viewer — a themed top-down arena canvas with moving shinobi circles (gold = player side), beat-driven clash choreography, KO markers, trails, final tableau. **Every league nation has a unique home venue** (`shared/constants/arenas.js`: Dunebowl, Frostring, Cinder Arena, …) + special layouts for the 6 mission specs, academy days, the Proving Forest (exam) and Grand Colosseum (tournament); custom villages hash to a stable nation theme (`hashArenaKey`). Renderer/animator in `client/js/pitchView.js` (`mountPitch`); hosted by `liveBattle.js` above the momentum bar. Mission reports now carry `spec`; the Youth Cup records `G._youthCupRun` and the Youth Academy panel got a ▶ Watch academy-day replay (`watchYouthCup`). Reduced-motion hides the pitch (classic text viewer). **Browser-verified in a driven session** (escort mission on The Long Road; matchday in the Cinder Arena; zero console errors). +10 arena tests.
