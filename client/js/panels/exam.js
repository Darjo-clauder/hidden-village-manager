...
function _seasonHistoryHtml() {
  ...
  if (!years.length) return `<div style="font-size:8px;color:var(--text-faint);margin-bottom:12px">No completed seasons yet — history appears after Year 1 ends (Month 12).</div>`
  return `<div style="font-size:10px;color:var(--gold);margin:14px 0 8px;text-transform:uppercase;letter-spacing:1px">Season History</div>
      ...
      ${years.map(y => {
        ...
        return `<div style="background:var(--bg);border:1px solid var(--border);border-left:2px solid ${finCol};padding:8px 10px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
            <span style="font-size:10px;color:var(--text-hi);font-weight:bold">Year ${y}</span>
            <span style="font-size:8px;color:var(--text-dim)">Tier ${snap.prestige}</span>
            <span style="font-size:8px;color:${finCol};margin-left:auto">Finished ${finish}${snap.kiaCount ? ` · <span style="color:var(--red)">${snap.kiaCount} fallen</span>` : ''}</span>
          </div>
          <div style="display:flex;gap:14px;flex-wrap:wrap;font-size:8px;color:var(--text-dim)">
...
export function rEx() {
  ...
  const offSeasonBanner = G.isOffSeason ? `<div style="background:var(--sunken);border:1px solid var(--gold);padding:6px 10px;margin-bottom:10px;font-size:8px;color:var(--gold)">⛄ Off-season — Months 1–3. Focus: recruitment, contract renewals, squad prep. Season begins Month 4.</div>` : ''
  const tabHtml = `${offSeasonBanner}<div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap">
    ${tabs.map(t => `<button class="btn${window._exTab === t ? ' act' : ''}" onclick="exTab('${t}')" style="font-size:9px;padding:3px 8px${t === 'war' ? ';color:var(--gold)' : ''}">${labels[t]}${t === 'war' && G.warSched ? ' ●' : ''}</button>`).join('')}
  </div>`
...
function _seasonTab() {
  ...
  if (!G.season?.table) {
    ...
    return review + `<div style="color:var(--text-dim);font-size:10px;padding:14px 0">The season begins in Month 4. Fixtures and standings will appear here.</div>`
  }
  ...
  const overview = `<div style="background:var(--sunken);border:1px solid var(--border);padding:11px 13px;margin-bottom:12px">
    <div style="font-size:8px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:8px">The Shinobi Year</div>
    <div style="display:flex;gap:6px;align-items:stretch;flex-wrap:wrap;font-size:8px">
      ${[
        ...
        <div style="font-size:9px;color:var(--text-hi);font-weight:bold">${t}</div>
        <div style="font-size:7px;color:var(--gold);margin:2px 0">${when}</div>
        <div style="font-size:7px;color:var(--text-dim);line-height:1.4">${d}</div>
      </div>${i < 3 ? '<div style="align-self:center;color:var(--border-hi);font-size:11px">→</div>' : ''}`).join('')}
    </div>
  </div>`
  ...
  const fixtureList = `<div style="border:1px solid var(--border);background:var(--bg);padding:9px;margin-bottom:12px">
    <div style="font-size:8px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:6px">${t('season.fixtures', { village: playerName })}</div>
    <div style="display:grid;gap:2px">
      ${fixtures.map(f => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;font-size:8px;padding:3px 6px;background:${next ? 'rgba(201,168,76,.08)' : 'transparent'};border-left:2px solid ${next ? 'var(--gold)' : 'transparent'}">
          ...
          ${done ? `<span style="color:${resCol};font-weight:bold;width:16px;text-align:center">${res || '·'}</span>`
                 : next ? '<span style="color:var(--gold);font-size:7px">NEXT ▸</span>'
                 : '<span style="color:var(--border-hi);font-size:7px">upcoming</span>'}
        </div>`
      }).join('')}
    </div>
  </div>`
...
function _exhibitionCard() {
  ...
  const cupTag = c => c === 'SF' ? '<span style="font-size:6px;border:1px solid var(--gold);color:var(--gold);padding:0 3px">🏆 SF</span>'
    : c === 'F' ? '<span style="font-size:6px;border:1px solid var(--gold);color:var(--gold);padding:0 3px">🏆 FINAL</span>' : ''
  const holder = inv ? `<div style="font-size:7px;color:${inv.champion === G.vName ? 'var(--green)' : 'var(--text-mid)'};margin-top:5px">🏆 Invitational holder (Y${inv.year}): <b>${inv.champion}</b>${inv.champion === G.vName ? ' — that’s you' : ` (you: ${inv.playerResult})`}</div>` : ''
  return `<div style="border:1px solid var(--border);background:var(--bg);padding:9px;margin-bottom:12px">
    <div style="font-size:8px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:6px">🏮 Off-Season Slate — minor nations</div>
    <div style="display:grid;gap:2px">
      ${ex.map(e => `<div style="display:flex;align-items:center;gap:8px;font-size:8px;padding:3px 6px">
        ...
        <span style="flex:1;color:var(--text-hi)">${e.opp} <span style="font-size:7px;color:var(--text-faint)">Tier ${e.tier}</span> ${cupTag(e.cup)}</span>
    ...
    <div style="font-size:7px;color:var(--text-faint);margin-top:5px">Friendlies (M1/M3) + the annual Invitational knockout (M2) — purses and morale, no league standing at stake.</div>
    ${holder}
  </div>`
}
...
function _titleRaceBanner(playerName, round, totalRounds) {
  ...
  return `<div style="display:flex;align-items:center;gap:14px;background:linear-gradient(90deg,#13100a,var(--bg));border:1px solid var(--border);border-left:3px solid ${posCol};padding:9px 13px;margin-bottom:12px">
    <div style="text-align:center;min-width:46px">
      ...
      <div style="font-size:6px;letter-spacing:1px;color:var(--text-dim);text-transform:uppercase">of ${st.total}</div>
    </div>
    <div style="flex:1">
      <div style="font-size:9px;color:var(--text-hi);font-weight:bold">${verdict}</div>
      <div style="font-size:7px;color:var(--text-dim);margin-top:2px">${st.phase} · ${st.roundsLeft} round${st.roundsLeft === 1 ? '' : 's'} left · Leaders: <span style="color:var(--gold)">${st.leader}</span> (${st.leaderPts} pts)</div>
    </div>
  </div>`
}
...
function _seasonReviewCard(playerName) {
  ...
  const cards = order.map(k => awards[k]).filter(a => a && a.name).map(a =>
    `<div style="flex:1;min-width:120px;border:1px solid var(--surface-3);border-left:2px solid var(--gold);padding:7px 9px">
      <div style="font-size:7px;letter-spacing:1px;color:var(--gold);text-transform:uppercase">${a.label}</div>
      <div style="font-size:10px;color:var(--text-hi);font-weight:bold;margin:2px 0">${a.name}</div>
      <div style="font-size:7px;color:var(--text-dim);line-height:1.4">${a.reason}</div>
    </div>`).join('')
  ...
  return `<div style="border:1px solid var(--gold);background:linear-gradient(160deg,#13100a,var(--bg));padding:13px;margin-bottom:14px">
    <div style="text-align:center;margin-bottom:10px">
      <div style="font-size:7px;letter-spacing:3px;color:var(--text-dim);text-transform:uppercase">Year ${hist.year} · Season Review</div>
      <div style="font-size:13px;color:var(--gold);font-weight:bold;margin-top:3px">🏆 ${hist.champion || 'No champion'}</div>
      <div style="font-size:8px;color:var(--text-dim);margin-top:2px">League champions</div>
      <div style="margin-top:7px;font-size:10px;color:${vCol};font-weight:bold">${playerName}: ${pos}${ord(pos)} of ${total} — ${verdict}</div>
    </div>
    ${cards ? `<div style="font-size:7px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:6px">Season Awards</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:11px">${cards}</div>` : ''}
    <div style="font-size:7px;letter-spacing:2px;color:var(--text-dim);text-transform:uppercase;margin-bottom:5px">Final Standings</div>
...
function _matchPreviewCard(names, schedule, round, resultsByRound, playerName) {
  ...
  if (!p) return `<div style="border:1px dashed var(--border);background:var(--bg);padding:9px;margin-bottom:12px;font-size:8px;color:var(--text-dim)">Bye round — no fixture this matchday.</div>`
  ...
  const formRow = (label, form) => `<div style="display:flex;align-items:center;gap:6px;font-size:7px;color:var(--text-dim)"><span style="width:54px">${label}</span>${form.length ? form.map(pip).join('') : '<span style="color:var(--border-hi)">no games yet</span>'}</div>`
  ...
  return `<div style="border:1px solid ${isDerby ? 'var(--red)' : 'var(--gold)'};background:var(--sunken);padding:11px 13px;margin-bottom:12px">
    ${isDerby ? `<div style="text-align:center;font-size:9px;letter-spacing:3px;color:var(--red);font-weight:bold;margin-bottom:6px">🔥 DERBY MATCH 🔥</div>` : ''}
    <div style="font-size:7px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:7px">▸ Next Matchday — Round ${round + 1}</div>
    <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:9px">
      <div style="flex:1;text-align:right"><div style="font-size:11px;color:var(--gold);font-weight:bold">${playerName}</div><div style="font-size:7px;color:var(--text-dim)">${p.playerPos}${p.playerPos===1?'st':p.playerPos===2?'nd':p.playerPos===3?'rd':'th'} place</div></div>
      <div style="font-size:8px;color:var(--text-dim);text-align:center">${p.home ? 'vs' : '@'}<div style="font-size:6px;color:var(--text-faint);margin-top:2px">${p.home ? 'home' : 'away'}</div></div>
      <div style="flex:1"><div style="font-size:11px;color:var(--text-hi);font-weight:bold">${p.opp}</div><div style="font-size:7px;color:var(--text-dim)">${p.oppPos}${p.oppPos===1?'st':p.oppPos===2?'nd':p.oppPos===3?'rd':'th'} place</div></div>
    </div>
    <div style="display:flex;flex-direction:column;gap:3px;border-top:1px solid var(--surface-3);padding-top:7px">
      ...
      <div style="font-size:7px;color:var(--text-dim);margin-top:1px">⚔ Head-to-head: <span style="color:var(--text-mid)">${h2hTxt}</span>${allTime ? ` · <span style="color:var(--text-mid)">${allTime}</span>` : ''}</div>
      ${(() => { const ace = (G.villages || []).find(v => v.n === p.opp)?.aces?.[0]; return ace ? `<div style="font-size:7px;color:var(--text-dim);margin-top:1px">⭐ Their ace: <span style="color:var(--text-hi)">${ace.name}</span> <span style="color:var(--text-faint)">(Pwr ${ace.pow})</span></div>` : '' })()}
      ${p.stakes.length ? `<div style="font-size:7px;color:var(--gold);margin-top:2px">At stake: ${p.stakes.join(' · ')}</div>` : ''}
    </div>
    ${_tacticPicker(p.opp)}
  </div>`
}
...
function _tacticPicker(oppName) {
  ...
  return `<div style="border-top:1px solid var(--surface-3);margin-top:7px;padding-top:7px">
    <div style="font-size:7px;color:var(--text-dim);margin-bottom:5px">They play <span title="${st.desc}" style="color:var(--gold);cursor:help">${st.icon} ${st.label}</span> — choose your approach:</div>
...
function _seasonFixtureGrid(names, schedule, round, resultsByRound, playerName) {
  ...
  return `<div style="border:1px solid var(--border);background:var(--bg);padding:9px;margin-bottom:12px">
    <div style="font-size:8px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:6px">${t('season.leagueGrid')}</div>
    <div style="display:grid;gap:8px;grid-template-columns:repeat(auto-fill,minmax(150px,1fr))">
      ${schedule.map((rnd, r) => {
        ...
        const tag = state === 'now' ? '<span style="color:var(--gold);font-size:7px">◂ NOW</span>' : state === 'done' ? '<span style="color:var(--border-hi);font-size:7px">played</span>' : '<span style="color:var(--border-hi);font-size:7px">upcoming</span>'
        return `<div style="border:1px solid ${state === 'now' ? 'var(--gold)' : 'var(--border-dim)'};border-left:2px solid ${state === 'now' ? 'var(--gold)' : 'var(--surface-3)'};padding:4px 0 5px">
          <div style="display:flex;justify-content:space-between;align-items:center;padding:0 6px 3px;font-size:7px;color:var(--text-dim);letter-spacing:1px">ROUND ${r + 1} ${tag}</div>
          ${rnd.length ? rnd.map(m => cell(m, r)).join('') : '<div style="font-size:7px;color:var(--border-hi);padding:2px 6px">— bye round —</div>'}
        </div>`
      }).join('')}
    </div>
    <div style="font-size:7px;color:var(--text-faint);margin-top:6px">Your village is highlighted gold. Winners in green; draws marked “=”.</div>
  </div>`
}

// Small colored form guide — last-5 W/D/L as pips, oldest→newest.
function _formPips(form) {
  if (!form.length) return '<span style="color:var(--border-hi);font-size:7px">—</span>'
  const col = { W: 'var(--green)', D: 'var(--gold)', L: 'var(--red)' }
  return form.map(r => `<span title="${r}" style="display:inline-block;width:9px;height:9px;line-height:9px;margin-left:2px;border-radius:2px;background:${col[r]};color:var(--bg);font-size:6px;text-align:center;font-weight:bold">${r}</span>`).join('')
}
...
function _seasonStandingsCard() {
  ...
  return `<div style="border:1px solid var(--border);background:var(--bg);padding:9px;margin-bottom:10px">
    <div style="font-size:8px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:6px">Season Standings — seeds the bracket</div>
    <table style="width:100%;border-collapse:collapse;font-size:8px">
      ...
      <tbody>${rows.map((row, i) => {
        ...
        const idTag = st ? ` <span title="${idn.label} — ${idn.blurb} Style: ${st.label} — ${st.desc}" style="font-size:7px;color:var(--text-dim);cursor:help">${st.icon}${st.label}</span>` : ''
...
function _seasonResultsCard(round, resultsByRound, playerName) {
  ...
  const line = m => {
    ...
    return `<div style="display:flex;align-items:center;gap:8px;font-size:8px;padding:3px 6px;background:${me ? 'rgba(201,168,76,.06)' : 'transparent'};border-left:2px solid ${me ? 'var(--gold)' : 'transparent'}">
  ...
  return `<div style="border:1px solid var(--border);background:var(--bg);padding:9px;margin-bottom:12px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
      <div style="font-size:8px;letter-spacing:2px;color:var(--gold);text-transform:uppercase">Matchday ${lastPlayed + 1} — Results</div>
      <div style="display:flex;align-items:center;gap:8px">
        ${star ? `<div style="font-size:7px;color:var(--text-dim)">★ Result of the round: <span style="color:var(--text-hi)">${star.m.winner} ${Math.max(star.m.scoreA, star.m.scoreB)}–${Math.min(star.m.scoreA, star.m.scoreB)}</span></div>` : ''}
        ${playerInRound ? `<button class="gb" style="font-size:7px;padding:2px 8px;border-color:var(--gold);color:var(--gold)" onclick="watchMatchday()">▶ Watch your match</button>` : ''}
      </div>
    </div>
...
function _renderExamSetup(el, tabHtml) {
  ...
  const formatBanner = fmt_
    ? `<div style="padding:8px 10px;border:1px solid var(--gold);background:var(--sunken);margin-bottom:10px">
        <div style="font-size:9px;color:var(--gold);font-weight:bold">${fmt_.icon} Exam Format: ${fmt_.n}</div>
        <div style="font-size:8px;color:var(--text-dim);margin-top:3px">${fmt_.desc}</div>
        <div style="font-size:8px;color:var(--green);margin-top:3px">Bonus: Nominees strong in <b>${fmt_.bonusStats.join(', ')}</b> gain +15% performance.</div>
      </div>`
    : `<div style="font-size:8px;color:var(--text-faint);margin-bottom:8px">Format will be revealed when you start.</div>`
  const seasonCard = _seasonStandingsCard()
  if (!G.examSched) {
    el.innerHTML = tabHtml + hostInfo + formatBanner + seasonCard + '<div style="color:var(--text-dim);font-size:10px">No exam scheduled.<br><br><button class="gb" onclick="schEx()">Request exam</button></div>'
    return
  }
  ...
  const quickFormBtn = spareCells > 0
    ? `<button class="gb" onclick="quickFormExamCells()" style="font-size:8px;padding:3px 8px;border-color:var(--green);color:var(--green);margin-bottom:10px">✚ Quick-form ${spareCells} academy cell${spareCells === 1 ? '' : 's'} & nominate</button>
       <div style="font-size:7px;color:var(--text-faint);margin-bottom:12px">Drafts your spare Initiate/Adept prospects into three-ninja cells and nominates them — no need to build squads first.</div>`
    : ''
  el.innerHTML = tabHtml + hostInfo + formatBanner + seasonCard +
    `<div style="font-size:9px;color:var(--text-dim);margin-bottom:12px">Nominate squads (max ${maxCands}). All members must be available. Promotable members (Initiate/Adept) earn promotion by advancing deep. Squads that reach the final build lasting cohesion.</div>` +
    quickFormBtn +
    ...
    (squads.length ? squads.map(({ sq, members }) => {
      ...
      const harmonyTag = harmony.bonus > 0 ? ` <span title="Coherent chakra natures — +${harmony.bonus} cohesion after the exam" style="color:var(--blue);font-size:8px">◈ ${harmony.label}</span>` : ''
      return `<div class="pi" onclick="tEC('${sq.id}')" style="${ent ? 'border-color:var(--gold);background:rgba(201,168,76,0.08)' : ''}"><div><div style="font-size:10px;color:var(--text-hi)">${sq.n}${sq.identity ? ` <span style="color:var(--gold);font-size:8px">«${sq.identity.title}»</span>` : ''}${fmtMatch ? ' <span style="color:var(--green);font-size:8px">★ format match</span>' : ''}${harmonyTag}</div><div style="font-size:8px;color:var(--text-dim)">${members.length} ninja · Pwr ${_squadPow(members, sq.cohesion)} · cohesion ${sq.cohesion ?? 0} · ${promotable} promotable</div><div style="font-size:7px;color:var(--text-faint)">${members.map(s => sn(s) + ' (' + RANKS[s.ri][0] + ')').join(', ')}</div></div>${ent ? '<span style="color:var(--gold)">✓</span>' : ''}</div>`
    }).join('') : '<div style="color:var(--text-dim);font-size:9px">No eligible squads. Build a squad in the Squads panel (all members must be available).</div>') +
    '</div>' +
    ...
    ((G._examRun && G._examRun.stages && G._examRun.stages.length)
      ? `<div style="margin-top:14px"><button class="gb" style="font-size:7px;padding:2px 8px;border-color:var(--gold);color:var(--gold)" onclick="watchExam()">▶ Watch your exam run</button></div>` : '') +
    (G.examResults.length ? '<div style="margin-top:14px;font-size:8px;color:var(--text-dim);letter-spacing:2px;text-transform:uppercase;margin-bottom:7px">Recent Results</div>' + G.examResults.slice(-10).map(r => `<div style="font-size:9px;padding:4px 0;border-bottom:1px solid var(--border);color:${r.promoted ? 'var(--green)' : 'var(--text-dim)'}">${r.name}: ${r.result}${r.promoted ? ' → Promoted!' : ''}</div>`).join('') : '')
}
...
function _hostBidCard(isHost) {
  if (isHost) return `<div style="font-size:9px;color:var(--green);margin-bottom:8px">🏠 Hosting this exam — home-crowd edge + gate revenue active</div>`
  // Bidding only offered while an exam is scheduled and not yet resolved.
  if (!G.examSched || G.examHostResolved) return `<div style="font-size:9px;color:var(--text-dim);margin-bottom:8px">Away exam</div>`
  const tier = G.prestigeTier
  if (!isHostEligible(tier)) return `<div style="font-size:9px;color:var(--text-dim);margin-bottom:8px">Away exam — hosting needs prestige C or higher.</div>`
  ...
  return `<div style="padding:9px 11px;border:1px solid var(--gold);background:var(--sunken);margin-bottom:10px">
    <div style="font-size:9px;color:var(--gold);font-weight:bold;margin-bottom:4px">🏛 Bid to Host the Adept Exam</div>
    <div style="font-size:8px;color:var(--text-dim);margin-bottom:6px">Win hosting rights for a home-crowd edge across the bracket and <b style="color:var(--green)">${(hostRevenue(tier)).toLocaleString()} ryo</b> gate revenue. Rival villages bid too — outbid them. Min bid ${min.toLocaleString()}.</div>
    <div style="display:flex;gap:6px;align-items:center">
      <input id="host-bid-input" type="number" value="${suggested}" min="${min}" step="1000" style="width:110px;background:var(--sunken);border:1px solid var(--border);color:var(--gold-hi);font-size:9px;padding:3px 5px">
...
function rExA(el, tabHtml) {
  ...
  const biasWarning = bias?.isBiased && !G.examJudgeProtested && r === 3
    ? `<div style="padding:7px;border:1px solid var(--orange);background:var(--sunken);margin-bottom:8px;font-size:8px;color:var(--orange)">⚠ Judge from ${bias.villageName} (rival) — nominee scores lowered by ${Math.round(bias.biasMod * 100)}%. <button class="gb" onclick="protestJudge()" style="font-size:7px;margin-left:8px;border-color:var(--orange);color:var(--orange)">File Protest (costs 5 prestige) ▸</button></div>`
    : ''
  ...
  el.innerHTML = tabHtml + biasWarning + `<div style="background:var(--bg);border:1px solid var(--gold);padding:14px">
    <div style="font-size:8px;letter-spacing:3px;color:var(--gold);text-transform:uppercase;margin-bottom:8px">${EXAM_STAGES[r] || 'Final'}</div>
    ${G.examFormat ? `<div style="font-size:8px;color:var(--text-dim);margin-bottom:8px">${G.examFormat.icon} Format: ${G.examFormat.n} — format-matched nominees get +15%</div>` : ''}
    <div style="font-size:8px;color:var(--text-dim);margin-bottom:8px">${standing.length} villages in contention · ${totalAlive} squads remaining</div>
    <div style="display:grid;gap:8px;margin-bottom:10px">
      ${standing.map((e, i) => `<div style="border:1px solid ${e.isPlayer ? 'var(--gold)' : 'var(--border)'};padding:7px;background:${e.isPlayer ? 'rgba(201,168,76,0.06)' : 'var(--bg)'}">
        <div style="font-size:9px;color:${e.isPlayer ? 'var(--gold)' : 'var(--text-mid)'};margin-bottom:4px">${i === 0 ? '◆ ' : ''}${e.seed ? `<span style="color:var(--text-dim)">#${e.seed}</span> ` : ''}${e.ico || '🏳'} ${e.name}${e.isPlayer ? ' (you)' : ''} <span style="color:var(--text-dim)">— ${e.alive.length} alive${e.scrolls != null ? ` · ${e.scrolls} 📜` : ''}</span></div>
        ${e.alive.map(c => `<div style="font-size:8px;padding:2px 0;color:${c.isPlayer ? 'var(--text-hi)' : 'var(--text-dim)'}">${c.name} · ${(c.members || []).length}-ninja · Pwr ${c.pow}${c._wounded ? ' <span style="color:var(--red-soft)">⚕ wounded</span>' : ''}${_squadFormatBonus(c) > 0 ? ' <span style="color:var(--green)">★+15%</span>' : ''}</div>`).join('')}
      </div>`).join('')}
    </div>
    <div style="margin:10px 0 8px">
      <div style="font-size:7px;letter-spacing:2px;color:var(--text-dim);text-transform:uppercase;margin-bottom:4px">Stage Posture — your squads only</div>
      <div style="display:flex;gap:5px">
        ${EXAM_POSTURES.map(p => {
          ...
          return `<div onclick="setExamPosture('${p.id}')" title="${p.desc}" style="flex:1;text-align:center;padding:5px 4px;cursor:pointer;border:1px solid ${sel ? 'var(--gold)' : 'var(--border)'};background:${sel ? 'rgba(201,168,76,.10)' : 'transparent'}">
            ...
            <div style="font-size:8px;color:${sel ? 'var(--gold)' : 'var(--text-mid)'};font-weight:${sel ? 'bold' : 'normal'}">${p.label}</div>
            <div style="font-size:6px;color:var(--text-faint);margin-top:1px">${p.adv > 0 ? '+' : ''}${Math.round(p.adv * 100)}% adv</div>
          </div>`
        }).join('')}
      </div>
    </div>
    ${!ui.exSt.sabotaged && r === 1 ? `<button class="btn" onclick="sabotageSquad()" style="font-size:9px;margin-bottom:8px;color:var(--red-soft)">Sabotage a rival squad (risk relations)</button> ` : ''}
...
function _renderSemifinalOverlay(duels, bye, field) {
  ...
  document.getElementById('ef-c').innerHTML =
    `<div style="font-size:9px;color:var(--text-dim);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px">${EXAM_STAGES[2]}</div>` +
    ...
    (duels.length ? duels.map(d => `
      <div style="border:1px solid var(--border);padding:6px;background:var(--bg)">
        ...
        <div style="font-size:7px;color:var(--text-dim);margin-top:2px">${d.discipline ? `<span style="color:var(--blue-hi)">${d.discipline}</span> · ` : ''}${d.marginLabel} decision${d.upset ? ' · <span style="color:var(--orange)">UPSET</span>' : ''}</div>
      </div>`).join('') : '<div style="font-size:9px;color:var(--text-dim)">No duels this round.</div>') +
    (bye ? `<div style="font-size:8px;color:var(--text-dim);padding:4px">${bye.c.name} (${bye.entry.name}) advances on a bye.</div>` : '') +
    `</div><div style="margin-top:8px;font-size:9px;color:var(--text-dim)">${villagesIn} villages reach the final · ${totalAlive} finalists.</div>`
...
function _renderRoundOverlay(r, res, field) {
  ...
  const totalAlive = field.reduce((a, e) => a + e.alive.length, 0)
  document.getElementById('ef-c').innerHTML =
    `<div style="font-size:9px;color:var(--text-dim);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px">${EXAM_STAGES[r]}</div>` +
    (res.length
      ? res.map(x => `<div style="font-size:9px;padding:4px 0;border-bottom:1px solid var(--border);color:${x.good ? 'var(--green)' : 'var(--red)'}"><span style="color:var(--gold)">${x.name}</span> — ${x.result}</div>`).join('')
      : '<div style="font-size:9px;color:var(--text-dim)">Your nominees saw no decisive action this stage.</div>') +
    `<div style="margin-top:8px;font-size:9px;color:var(--text-dim)">${villagesIn} villages still in contention · ${totalAlive} squads advance.</div>`
  document.getElementById('ov-examfight').classList.add('open'); upUI()
}
...
function _runFinals(field, biasMod) {
  ...
  const champLine = champ
    ? `<div style="font-size:11px;color:${playerWon ? 'var(--gold)' : 'var(--text-mid)'};margin-bottom:10px">🏆 Champion: ${champ.ico || ''} ${champ.name}${playerWon ? ' (you)' : ''}</div>`
    : ''
  const cohesionBlock = cohesionNotes.length
    ? `<div style="margin-top:12px;font-size:8px;color:var(--text-dim);letter-spacing:2px;text-transform:uppercase;margin-bottom:5px">Squad Cohesion Earned</div>` +
      cohesionNotes.map(n => `<div style="font-size:8px;padding:3px 0;border-bottom:1px solid #1e1a14;color:var(--text-mid)"><span style="color:var(--text-hi)">${n.name}</span> fought ${n.reach} — <span style="color:var(--green)">+${n.gain} cohesion</span> <span style="color:var(--text-faint)">(now ${n.cohesion})</span>${n.harmony ? ` <span style="color:var(--blue)">· ${n.harmony}</span>` : ''}</div>`).join('')
    : ''
  document.getElementById('ef-c').innerHTML = champLine +
    '<div style="font-size:9px;color:var(--text-dim);margin-bottom:10px">Exam complete!</div>' +
    (res.length
      ? res.map(x => `<div style="font-size:9px;padding:4px 0;border-bottom:1px solid var(--border);color:${x.promoted ? 'var(--green)' : 'var(--text-hi)'}"><span style="color:var(--gold)">${x.name}</span> — ${x.result}</div>`).join('')
      : '<div style="font-size:9px;color:var(--text-dim)">Your nominees were eliminated before the final.</div>') +
    cohesionBlock
  document.getElementById('ov-examfight').classList.add('open'); upUI()
}
...
function _summitTab() {
  ...
  if (bloc) {
    html += `<div style="border:1px solid var(--blue);background:#050d14;padding:10px;margin-bottom:14px">
      <div style="font-size:9px;color:var(--blue);font-weight:bold;margin-bottom:4px">⚑ Voting Bloc Offer from ${bloc.villageName}</div>
      <div style="font-size:8px;color:var(--text-dim);margin-bottom:8px">${bloc.villageName} will vote with you on <b>${bloc.agendaItem}</b> at the upcoming summit. In return, you owe them a future diplomatic favor.</div>
      <div style="display:flex;gap:8px">
        <button class="gb gb-g" onclick="acceptSummitBloc()" style="font-size:8px">Align — Secure Their Vote ▸</button>
        <button class="gb" onclick="declineSummitBloc()" style="font-size:8px;border-color:var(--text-faint);color:var(--text-faint)">Stay Independent ▸</button>
      </div>
    </div>`
  }
  if (!history.length) return html + '<div style="color:var(--text-faint);font-size:11px;padding:20px 0">No summits held yet. First summit occurs in month 6 of Year 1.</div>'
  html += history.slice().reverse().map(s => `
    <div class="ke-card" style="margin-bottom:8px">
      <div style="font-size:10px;color:var(--gold);margin-bottom:6px">Year ${s.year} Five Warden Summit${s.blocAligned ? ` <span style="font-size:8px;color:var(--blue)">[bloc: ${s.blocAligned}]</span>` : ''}</div>
      ${(s.results || []).map(r => `<div style="font-size:9px;padding:3px 0;border-bottom:1px solid var(--surface);color:${r.passed ? 'var(--green)' : 'var(--text-faint)'}">${r.passed ? '✓' : '✗'} ${r.item} ${r.myVote ? '(your vote: yes)' : '(your vote: no)'}</div>`).join('')}
    </div>`).join('')
  return html
}
...
function _srankTab() {
  ...
  if (!contracts.length) return '<div style="color:var(--text-faint);font-size:11px;padding:20px 0">No S-rank contracts posted this season. New contracts post monthly (months 1, 4, 7, 10).</div>'
  const now = (G.year - 1) * 12 + G.month
  return `<div style="font-size:9px;color:var(--text-dim);margin-bottom:10px">Competitive S-rank contracts. Win to earn ryo, reputation, and legend.</div>
  <div style="display:grid;gap:8px">
    ${contracts.map(c => {
      ...
      return `<div class="ke-card">
        <div style="font-size:11px;color:var(--text-hi);margin-bottom:6px">${c.n}</div>
        <div style="font-size:9px;color:var(--text-dim);margin-bottom:4px">Reward: ${fmt(c.baseRyo)} ryo · +${c.rep} rep · +${c.prestige} legend</div>
        <div style="font-size:9px;color:var(--text-dim);margin-bottom:8px">Risk: ${Math.round(c.risk * 100)}% failure chance · Deadline: M${c.deadline % 12 || 12}</div>
        ${won ? '<div style="font-size:10px;color:var(--green);margin-bottom:6px">✓ Won this contract!</div>' : ''}
        ${expired && !won ? '<div style="font-size:9px;color:var(--text-faint)">Contract expired.</div>' : ''}
        ${!expired && !won && !myBid ? `<button class="gb" onclick="bidSrank('${c.id}')" style="font-size:9px">Submit Bid</button>` : ''}
        ${myBid ? `<div style="font-size:9px;color:var(--gold)">Bid submitted — awaiting result.</div>` : ''}
      </div>`
    }).join('')}
  </div>`
}
...
function _recordsTab() {
  ...
  return `<div>
    ${champ ? `<div style="border:1px solid ${champ.player ? 'var(--gold)' : 'var(--border)'};background:${champ.player ? 'var(--sunken)' : 'var(--bg)'};padding:10px;margin-bottom:12px">
      <div style="font-size:8px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Reigning Champion (Year ${champ.year})</div>
      <div style="font-size:13px;color:${champ.player ? 'var(--gold)' : 'var(--text-hi)'}">🏆 ${champ.ico || ''} ${champ.village}${champ.player ? ' — your village' : ''}</div>
    </div>` : ''}
    ${_seasonHistoryHtml()}
    <div style="font-size:10px;color:var(--gold);margin-bottom:10px;text-transform:uppercase;letter-spacing:1px">Historical Exam Records</div>
    <div style="display:grid;gap:4px;margin-bottom:14px">
      ${rows.map(r => `<div style="display:flex;justify-content:space-between;padding:5px 8px;background:var(--bg);border-radius:3px">
        <span style="font-size:9px;color:var(--text-dim)">${r.label}</span>
        <span style="font-size:10px;color:var(--text-hi);font-weight:bold">${r.value}</span>
      </div>`).join('')}
    </div>
    ${champLeaders.length ? `<div style="font-size:10px;color:var(--gold);margin-bottom:8px;text-transform:uppercase;letter-spacing:1px">Championship Titles</div>
      <div style="display:grid;gap:3px;margin-bottom:14px">${champLeaders.map(([v, n]) => `<div style="display:flex;justify-content:space-between;padding:4px 8px;background:var(--bg);border-radius:3px"><span style="font-size:9px;color:${v === G.vName ? 'var(--gold)' : 'var(--text-dim)'}">${v}${v === G.vName ? ' (you)' : ''}</span><span style="font-size:10px;color:var(--text-hi);font-weight:bold">${n}</span></div>`).join('')}</div>` : ''}
    ${upsets.length ? `<div style="font-size:10px;color:var(--orange);margin-bottom:8px;text-transform:uppercase;letter-spacing:1px">Recorded Upsets</div>
      ${upsets.slice().reverse().map(u => `<div style="font-size:8px;color:var(--text-dim);padding:5px 8px;background:var(--bg);margin-bottom:4px;border-left:2px solid var(--orange)">Year ${u.year}: ${u.desc}</div>`).join('')}` : '<div style="font-size:8px;color:var(--text-faint)">No upsets recorded yet — outperform expectations at D or C prestige to create one.</div>'}
  </div>`
}
...
function _leadersTab() {
  ...
  const section = (title, rows) => `<div style="font-size:10px;color:var(--gold);margin:12px 0 6px;text-transform:uppercase;letter-spacing:1px">${title}</div>${rows}`
  const leaderTable = `<table style="width:100%;border-collapse:collapse">
    ...
    <tbody>${rows.map((s, i) => `<tr style="background:${i % 2 === 0 ? 'var(--sunken)' : 'var(--sunken)'};border-bottom:1px solid var(--sunken)">
      ${_LEADER_COLS.map(c => `<td style="padding:4px 6px;font-size:9px;text-align:${c.align};color:var(--text-hi)">${c.render(s)}</td>`).join('')}
    </tr>`).join('')}</tbody>
  </table>`
  return `<div>
    <div style="font-size:7px;color:var(--border-hi);margin-bottom:4px">League leaders — click a column to sort</div>
    ${leaderTable}
    ${G.seasonAwards && Object.keys(G.seasonAwards).length ? (() => {
      ...
      return section('Year ' + lastYear + ' awards', Object.values(awards).filter(a => a?.name).map(a =>
        `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 8px;background:var(--bg);border-radius:3px;margin-bottom:3px">
          <span style="font-size:9px;color:var(--text-hi)">${a.label}</span>
          <span style="font-size:9px;color:var(--gold)">${a.name}</span>
        </div>`).join(''))
    })() : ''}
  </div>`
}
