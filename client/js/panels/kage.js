...
export function rKa() {
  ...
  const vH = G.villages.map(v => {
    ...
    const idLine = idn.id !== 'none' ? `<div style="font-size:8px;color:var(--text-mid);margin-bottom:6px;font-style:italic" title="${st.label}: ${st.desc}">${st.icon} <b style="color:var(--gold);font-style:normal">${idn.label}</b> — ${idn.blurb}</div>` : ''
    const ace = v.aces?.[0]
    const aceLine = ace ? `<div style="font-size:8px;color:var(--text-dim);margin-bottom:6px">⭐ Ace: <b style="color:var(--text-hi)">${ace.name}</b> <span style="color:var(--text-faint)">(Pwr ${ace.pow})</span>${v.aces[1] ? ` · ${v.aces[1].name}` : ''}</div>` : ''
    const isDerby = v.n === G.derbyRival
    const derbyChip = isDerby ? `<span style="font-size:7px;border:1px solid var(--red);color:var(--red);padding:0 4px;margin-left:5px">🔥 DERBY RIVAL</span>` : ''
    const atLabel = h2hLabel(G.h2h, v.n)
    const h2hLine = atLabel ? `<div style="font-size:8px;color:var(--text-dim);margin-bottom:6px">⚔ League record vs you: <span style="color:var(--text-mid)">${atLabel}</span></div>` : ''
    return `<div class="ke-card"><div style="display:flex;align-items:center;gap:8px;margin-bottom:5px"><div style="font-size:20px">${v.ico}</div><div><div style="font-size:11px;color:var(--text-hi);font-weight:bold">${v.n}</div><div style="font-size:8px;color:var(--text-dim)">${v.kageRank} ${v.kage} · <span style="color:${rc}">${v.rel > 60 ? 'Allied' : v.rel > 30 ? 'Neutral' : 'Hostile'}</span>${v.allied ? ' ✓ Allied' : ''}${derbyChip}</div></div></div>${idLine}${aceLine}${h2hLine}<div style="display:flex;align-items:center;gap:7px;margin-bottom:3px"><div style="font-size:7px;color:var(--text-dim);width:60px;text-transform:uppercase;letter-spacing:1px">Relations</div><div class="bar" style="flex:1"><div class="fill" style="width:${v.rel}%;background:${rc}"></div></div><div style="font-size:9px;color:var(--text-dim)">${v.rel}</div></div><div style="display:flex;align-items:center;gap:7px;margin-bottom:6px"><div style="font-size:7px;color:var(--text-dim);width:60px;text-transform:uppercase;letter-spacing:1px">Strength</div><div class="bar" style="flex:1"><div class="fill" style="width:${Math.min(100,vs/2)}%;background:${strColor}"></div></div><div style="font-size:8px;color:${strColor}">${strLabel} (${Math.round(vs)})</div></div><div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap"><button class="gb gb-b" onclick="sGift('${v.n}')" ${G.ryo < 5000 ? 'disabled' : ''}>Send gifts +10 (5k ryo)</button>${v.rel > 60 && !v.allied ? `<button class="gb gb-g" onclick="propAl('${v.n}')" ${G.ryo < 10000 ? 'disabled' : ''}>Propose alliance (10k)</button>` : ''}${v.rel < 60 && !v.allied ? `<button class="gb" onclick="demandTribute('${v.n}')" title="Strength-gated. Success extracts ryo; failure angers them.">Demand tribute</button>` : ''}${(v.threat || 0) > 0 ? `<button class="gb gb-b" onclick="appease('${v.n}')" ${G.ryo < 4000 ? 'disabled' : ''}>Appease −threat (4k)</button>` : ''}${v.rel < 30 ? `<button class="gb gb-r" onclick="rattle('${v.n}')">Rattle sabres</button>` : ''}<button class="gb" onclick="sabotageRival('${v.n}')" ${G.ryo < RIVAL_OP_COST ? 'disabled' : ''} title="Covert op (${fmt(RIVAL_OP_COST)} ryo): dent their strength so they slip in the league. Failure exposes you.">🗡 Disrupt (${fmt(RIVAL_OP_COST)})</button></div></div>`
  }).join('')
  ...
  const standingsHtml = `<div class="ke-card" style="margin-bottom:14px">
    <div style="font-size:9px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:8px">${tr("kage.powerStandings")}</div>
  ...
  el.innerHTML = (ui.pKE
    ? `<div class="ke-card" style="border-color:var(--gold);margin-bottom:14px"><div style="font-size:9px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:8px">⚡ Warden Event</div><div style="font-size:12px;color:var(--text-hi);font-weight:bold;margin-bottom:5px">${ui.pKE.n}</div><div style="font-size:10px;color:var(--text-dim);margin-bottom:12px;line-height:1.5">${ui.pKE.desc}</div><div style="display:flex;flex-direction:column;gap:6px">${ui.pKE.choices.map((c, i) => `<button class="gb" onclick="resKE(${i})">${c.l}</button>`).join('')}</div></div>`
...
function _rivalDemandHtml() {
  ...
  return `<div class="ke-card" style="border-color:var(--red);background:#160808;margin-bottom:14px">
    <div style="font-size:9px;letter-spacing:2px;color:var(--red);text-transform:uppercase;margin-bottom:6px">⚠ Tribute Demand — ${_diploQuarter()}</div>
    <div style="font-size:11px;color:var(--text-hi);margin-bottom:8px">${d.icon || ''} <b>${d.villageName}</b> demands <b style="color:var(--orange)">${fmt(d.amount)} ryo</b> in tribute, or relations sour and their war footing grows.</div>
...
function _mandateHtml() {
  ...
  const mandateRows = m.ids.map(id => {
    ...
    return `<div style="padding:5px 0;border-bottom:1px solid var(--sunken)">
      <div style="font-size:9px;color:var(--text-hi)">${def.n}</div>
      <div style="font-size:8px;color:var(--text-dim)">${def.desc}</div>
      <div style="font-size:8px;color:var(--text-faint);margin-top:2px">+${def.confidenceGain} if met · -${def.confidenceLoss} if missed</div>
    </div>`
  }).join('')
  const history = (m.history || []).slice(-3).reverse().map(h => {
    ...
    return `<div style="font-size:8px;color:var(--text-dim);padding:2px 0">Y${h.year}: ${icons} → confidence <span style="color:${c}">${net}</span> → ${h.confidenceAfter}</div>`
  }).join('')
  ...
  return `<div class="ke-card" style="border-color:${confColor}44;margin-bottom:14px">
    ...
    <div style="margin-bottom:10px">
      ...
      ${conf < DISMISSAL_THRESHOLD
        ? `<div style="font-size:8px;color:var(--red);margin-top:4px">⚠ Confidence critically low. Two consecutive bad years triggers a no-confidence vote.</div>`
        : `<div style="font-size:8px;color:var(--text-faint);margin-top:4px">Evaluated each December. Drops below ${DISMISSAL_THRESHOLD} for 2+ years → no-confidence vote.</div>`}
    </div>
    ${mandateRows}
    ${history ? `<div style="margin-top:8px;border-top:1px solid var(--border-dim);padding-top:6px"><div style="font-size:7px;color:var(--text-faint);text-transform:uppercase;letter-spacing:.1em;margin-bottom:3px">${tr("kage.recentHistory")}</div>${history}</div>` : ''}
  </div>`
}
...
function _noConfidenceHtml() {
  ...
  return `<div class="ke-card" style="border-color:var(--red);background:rgba(255,68,68,0.06);margin-bottom:14px">
    <div style="font-size:10px;letter-spacing:2px;color:var(--red);text-transform:uppercase;margin-bottom:8px">⚠ No-Confidence Vote</div>
    <div style="font-size:11px;color:var(--text-hi);margin-bottom:8px">The council demands a change of leadership after consecutive mandate failures.</div>
    <div style="font-size:9px;color:var(--text-dim);margin-bottom:12px">You may resign with honour or spend 15,000 ryo to rally council support and fight the vote.</div>
...
function _philosophyHtml() {
  ...
  return `<div class="ke-card" style="margin-bottom:14px">
    <div style="font-size:9px;letter-spacing:2px;color:var(--gold);text-transform:uppercase;margin-bottom:8px">${tr("kage.coachingPhilosophy")}</div>
    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px">${buttons}</div>
    <div style="font-size:9px;color:var(--text-hi);margin-bottom:3px">${p.n} — ${p.desc}</div>
    ${modLine ? `<div style="font-size:8px;color:var(--text-dim)">${modLine}</div>` : `<div style="font-size:8px;color:var(--text-faint)">${tr("kage.noStatMods")}</div>`}
  </div>`
}
