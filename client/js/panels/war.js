...
function _renderWarMuster(el, tabHtml) {
  ...
  const recap = last ? `<div style="border:1px solid ${last.champ?.player ? 'var(--gold)' : '#5a2a2a'};background:#0d0606;padding:10px;margin-bottom:12px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
        <div style="font-size:var(--fs-lead);color:${last.champ?.player ? 'var(--gold)' : '#e8a0a0'}">${last.champ ? `🏯 Last Grand Tournament — Champion: ${last.champ.ico || ''} ${last.champ.name}${last.champ.player ? ' (you)' : ''}` : 'Grand Tournament concluded.'}</div>
        ${canWatch ? `<button class="gb" style="font-size:var(--fs-micro);padding:2px 8px;border-color:var(--gold);color:var(--gold);white-space:nowrap" onclick="watchTournament()">▶ Watch your run</button>` : ''}
      </div>
      ${last.fallen ? `<div style="font-size:var(--fs-small);color:var(--red-soft);margin-top:4px">${last.fallen} of our shinobi fell.</div>` : ''}
    </div>` : ''
  ...
  return (el.innerHTML = tabHtml + recap +
    `<div style="border:1px solid var(--gold);background:var(--sunken);padding:10px;margin-bottom:12px">
      <div style="font-size:var(--fs-lead);color:var(--gold)">🏯 The Grand Tournament is mobilizing!</div>
      <div style="font-size:var(--fs-small);color:var(--text-dim);margin-top:4px">Muster up to ${WAR_MAX_SQUADS} elite squads (Veteran+). Casualties are expected — vessel and bloodline clans survive more often.</div>
    </div>` +
    ...
    (elite.length ? elite.map(({ sq, members }) => {
      ...
      return `<div class="pi" onclick="musterWar('${sq.id}')" style="${ent ? 'border-color:var(--gold);background:rgba(201,168,76,0.08)' : ''}"><div><div style="font-size:var(--fs-body);color:var(--text-hi)">${sq.n}${sq.identity ? ` <span style="color:var(--gold);font-size:var(--fs-small)">«${sq.identity.title}»</span>` : ''}</div><div style="font-size:var(--fs-small);color:var(--text-dim)">${members.length} elite · Pwr ${_squadPow(members, sq.cohesion)} · cohesion ${sq.cohesion ?? 0}${protCount ? ` · <span style="color:var(--green)">${protCount} protected</span>` : ''}</div><div style="font-size:var(--fs-micro);color:var(--text-faint)">${members.map(s => sn(s) + ' (' + RANKS[s.ri][0] + ')').join(', ')}</div></div>${ent ? '<span style="color:var(--gold)">✓</span>' : ''}</div>`
    }).join('') : '<div style="color:var(--text-dim);font-size:var(--fs-body)">No eligible elite squads. Form a squad of all-Veteran+ shinobi (all available) in the Squads panel.</div>') +
    '</div>' +
...
function _renderActiveWar(el, tabHtml) {
  ...
  el.innerHTML = tabHtml + `<div style="background:#0d0808;border:1px solid var(--gold);padding:14px">
    <div style="font-size:var(--fs-small);letter-spacing:3px;color:var(--gold);text-transform:uppercase;margin-bottom:8px">Grand Tournament — ${WAR_STAGES[r] || 'Final Stand'}</div>
    <div style="font-size:var(--fs-small);color:var(--text-dim);margin-bottom:8px">${standing.length} villages in the field · ${totalAlive} squads standing · <span style="color:var(--red-soft)">${totalFallen} fallen</span></div>
    <div style="display:grid;gap:8px;margin-bottom:10px">
      ${standing.map((e, i) => `<div style="border:1px solid ${e.isPlayer ? 'var(--gold)' : 'var(--border)'};padding:7px;background:${e.isPlayer ? 'rgba(201,168,76,0.06)' : 'var(--bg)'}">
        <div style="font-size:var(--fs-body);color:${e.isPlayer ? 'var(--gold)' : 'var(--text-mid)'};margin-bottom:4px">${i === 0 ? '◆ ' : ''}${e.seed ? `<span style="color:var(--text-dim)">#${e.seed}</span> ` : ''}${e.ico || '🏳'} ${e.name}${e.isPlayer ? ' (you)' : ''} <span style="color:var(--text-dim)">— ${e.alive.length} squads${e.fallen?.length ? ` · <span style="color:var(--red-soft)">${e.fallen.length} dead</span>` : ''}</span></div>
        ${e.alive.map(c => `<div style="font-size:var(--fs-small);padding:2px 0;color:${c.isPlayer ? 'var(--text-hi)' : 'var(--text-dim)'}">${c.name} · ${(c.members || []).length}-ninja · Pwr ${c.pow}</div>`).join('')}
      </div>`).join('')}
    </div>
    <div style="margin-bottom:10px">
      <div style="font-size:var(--fs-micro);letter-spacing:2px;color:var(--text-dim);text-transform:uppercase;margin-bottom:4px">Command Order — your squads only</div>
      <div style="display:flex;gap:5px">
        ${WAR_COMMANDS.map(cmd => {
          ...
          return `<div onclick="setWarCommand('${cmd.id}')" title="${cmd.desc}" style="flex:1;text-align:center;padding:5px 4px;cursor:pointer;border:1px solid ${sel ? 'var(--gold)' : 'var(--border)'};background:${sel ? 'rgba(201,168,76,.10)' : 'transparent'}">
            <div style="font-size:var(--fs-lead)">${cmd.icon}</div>
            <div style="font-size:var(--fs-small);color:${sel ? 'var(--gold)' : 'var(--text-mid)'};font-weight:${sel ? 'bold' : 'normal'}">${cmd.label}</div>
            <div style="font-size:var(--fs-micro);color:var(--text-faint);margin-top:1px">${cmd.adv > 0 ? '+' : ''}${Math.round(cmd.adv * 100)}% · ${cmd.kiaMult < 1 ? '−' : cmd.kiaMult > 1 ? '+' : ''}${Math.abs(Math.round((cmd.kiaMult - 1) * 100))}% KIA</div>
          </div>`
        }).join('')}
      </div>
    </div>
