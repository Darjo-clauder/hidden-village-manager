...
export function rTr() {
  document.getElementById('ec-trade').innerHTML = G.tradeRoutes.map(r => {
    ...
    return `<div class="tr-card" style="${r.disrupted ? 'border-left:2px solid #f66' : ''}"><div style="font-size:11px;color:#e8e0cc;font-weight:bold;margin-bottom:3px">${r.n}${statusTag}${disruptedTag}</div><div style="font-size:9px;color:#7a7060;margin-bottom:7px">${r.desc}${bl ? ` <span style="color:#f66">(Requires ${r.req === 'jk' ? 'Vessel' : 'Intel Network'})</span>` : ''}</div><div style="display:flex;gap:14px;font-size:8px;color:#7a7060;margin-bottom:7px">${r.cost > 0 ? `<span>Setup: <span style="color:#e8e0cc">${fmt(r.cost)} ryo</span></span>` : ''}${incomeLine}</div>${r.disrupted ? `<button class="gb gb-g" onclick="secureRoute('${r.id}')" ${G.ryo < secureCost ? 'disabled' : ''}>Secure Route — ${fmt(secureCost)} ryo ►</button> ` : ''}${r.active ? `<button class="gb gb-r" onclick="tgTr('${r.id}',false)">${tr("economy.closeRoute")}</button>` : `<button class="gb gb-g" onclick="tgTr('${r.id}',true)" ${G.ryo < r.cost || bl ? 'disabled' : ''}>Open Route${r.cost > 0 ? ' — ' + fmt(r.cost) + ' ryo' : ' (free)'} ►</button>`}</div>`
  }).join('')
}
