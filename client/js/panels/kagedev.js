...
export function rKageDev() {
  ...
  const header = `<div style="background:var(--surface);border:1px solid var(--accent-border);border-top:2px solid var(--accent);padding:13px 15px;margin-bottom:14px">
    <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap">
      <span style="font-size:var(--fs-head);color:var(--accent);font-weight:bold">${G.kName || 'Warden'}</span>
      <span style="font-size:var(--fs-body);color:var(--text-dim)">Level ${k.level}${path ? ` · ${path.icon} ${path.n}` : ''}</span>
      ${k.points > 0 ? `<span style="margin-left:auto;font-size:var(--fs-body);color:var(--green)">● ${k.points} development point${k.points !== 1 ? 's' : ''} to spend</span>` : `<span style="margin-left:auto;font-size:var(--fs-small);color:var(--text-faint)">${t('kagedev.noPoints')}</span>`}
    </div>
  ...
  const pathHtml = `<div style="margin-bottom:16px">
    <div style="font-size:var(--fs-micro);letter-spacing:2px;color:var(--text-dim);text-transform:uppercase;margin-bottom:8px">Warden Path ${path ? `<span style="color:var(--accent);margin-left:6px">${path.icon} ${path.n} — chosen</span>` : '<span style="color:var(--orange);margin-left:6px">choose your background (permanent)</span>'}</div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:8px">
      ${KAGE_PATHS.map(p => {
        ...
        return `<div style="border:1px solid ${sel ? 'var(--accent)' : 'var(--border)'};background:${sel ? 'var(--accent-bg)' : 'transparent'};padding:10px;opacity:${locked ? 0.4 : 1}">
          <div style="font-size:var(--fs-body);color:${sel ? 'var(--accent)' : 'var(--text-hi)'};font-weight:bold;margin-bottom:3px">${p.icon} ${p.n}</div>
          <div style="font-size:var(--fs-small);color:var(--text-dim);line-height:1.5;margin-bottom:7px">${p.desc}</div>
          ${sel ? '<div style="font-size:var(--fs-small);color:var(--accent)">✓ Active</div>'
                : k.path ? '<div style="font-size:var(--fs-micro);color:var(--text-faint)">— not chosen</div>'
                : `<button class="gb gb-g" style="font-size:var(--fs-micro)" onclick="chooseKagePath('${p.id}')">Choose ▸</button>`}
        </div>`
      }).join('')}
    </div>
  </div>`
  ...
  // ── Attributes ─────────────────────────────────────────────────────────────
  const attrsHtml = `<div>
    <div style="font-size:var(--fs-micro);letter-spacing:2px;color:var(--text-dim);text-transform:uppercase;margin-bottom:8px">${t('kagedev.attributes')}</div>
