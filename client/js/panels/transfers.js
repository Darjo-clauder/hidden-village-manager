...
export function rTr() {
  ...
  el.innerHTML = `
    <!-- Window status bar -->
    <div style="background:${tm.windowOpen ? 'var(--green-bg)' : 'var(--surface)'};border:1px solid ${tm.windowOpen ? 'var(--green-hi)' : 'var(--border)'};border-radius:6px;padding:10px 14px;margin-bottom:16px;display:flex;align-items:center;gap:12px">
      <span style="font-size:1.2rem">${tw?.icon || '🗓'}</span>
      <div style="flex:1">
        <div style="font-size:.85rem;color:${tm.windowOpen ? 'var(--green)' : 'var(--text-dim)'};font-weight:bold">
      ...
      ${tm.windowOpen
        ? `<button onclick="refreshTransferPool()" style="background:var(--gold-bg);border:1px solid #554;color:var(--gold);border-radius:4px;padding:4px 10px;cursor:pointer;font-size:.75rem">${t("transfers.refreshPool")}</button>`
        : ''}
    </div>
...
function marketCard(p, judgeLevel) {
  ...
  const matTraits = showMatrix ? ['loyalty','ambition','professionalism','temperament','adaptability'].map(k => {
    ...
    return `<span style="font-size:.68rem;color:${p.pMatrix[k] >= 13 ? 'var(--green)' : p.pMatrix[k] >= 8 ? 'var(--text-mid)' : 'var(--red-soft)'}">${k.slice(0,3).toUpperCase()}: ${d}</span>`
  }).join('  ') : ''
  ...
  return `<div style="background:var(--surface);border:1px solid ${catDef.color}33;border-radius:6px;padding:12px;border-top:2px solid ${catDef.color}" oncontextmenu="return trCtx(event,'${p.id}')">
    <div style="display:flex;align-items:start;justify-content:space-between;margin-bottom:6px">
      <div onmousemove="trHover(event,'${p.id}')" onmouseleave="hideHoverPreview()">
        ...
        <div style="font-size:.72rem;color:var(--text-dim)">${RANKS[p.ri]} · ${p.clan || p.spec} · Age ${p.age}</div>
      </div>
      <span style="font-size:.72rem;background:var(--sunken);border:1px solid ${catDef.color};color:${catDef.color};border-radius:3px;padding:2px 6px">${catDef.icon} ${catDef.n}</span>
    </div>
    ${p.originVillage ? `<div style="font-size:.72rem;color:var(--text-dim);margin-bottom:4px">From: ${p.originVillage}</div>` : ''}
    <div style="font-size:.75rem;color:var(--text-dim);margin-bottom:6px">${catDef.desc}</div>
    <div style="font-size:.75rem;color:var(--text-mid);margin-bottom:8px">
      Power: <strong>${sPow(p)}</strong> · Pot: <strong>${p.potential}</strong>
      · Avail: <span style="color:${(p.monthsAvailable||1)<=1?'var(--red)':'var(--text-mid)'}">${p.monthsAvailable||1}mo</span>
    </div>
    <div style="font-size:.72rem;color:var(--text-dim);margin-bottom:6px">Market value: <span style="color:${valueDelta>=0?'var(--green)':'var(--orange)'}">${fmt(marketVal)} ryo</span> ${valueDelta>=0?'(bargain at asking price)':'(asking above value)'}</div>
    ${p.agent ? (() => {
      ...
      if (ag && tier.tip) {
        ...
        if (other) tip = `<div style="font-size:.68rem;color:var(--green);margin:2px 0 6px">🤝 First refusal: ${ag.name} also reps <b>${other.fn} ${other.ln}</b> — a quiet tip for a trusted partner.</div>`
      }
      return `<div style="font-size:.71rem;color:var(--purple);margin-bottom:6px">🤝 Agent: ${p.agent.name} (${p.agent.feePercent}% of signing bonus) — ${p.agent.agendaDesc}${standingTag}</div>${tip}`
    })() : ''}
    ...
    <div style="display:flex;gap:6px">
      ${canDirectSign
        ? `<button onclick="openPersonalTerms('${p.id}')" ${!signable?'disabled':''} style="flex:1;background:${signable?'var(--green-bg)':'var(--sunken)'};border:1px solid ${signable?'var(--green-hi)':'var(--border)'};color:${signable?'var(--green)':'var(--text-faint)'};border-radius:4px;padding:5px;cursor:${signable?'pointer':'not-allowed'};font-size:.75rem">
            ✓ Sign Direct
          </button>`
        : `<button onclick="openNegotiation('${p.id}')" ${!signable?'disabled':''} style="flex:1;background:var(--surface-2);border:1px solid var(--blue);color:var(--blue);border-radius:4px;padding:5px;cursor:pointer;font-size:.75rem">
            ⇄ Negotiate
          </button>
          <button onclick="poachAttempt('${p.id}')" style="background:var(--red-bg);border:1px solid var(--red);color:var(--red-soft);border-radius:4px;padding:5px;cursor:pointer;font-size:.75rem" title="Approach directly — cheaper but risks diplomatic incident">
            🕵 Poach
          </button>`
      }
    </div>
...
function renderSellPressure() {
  ...
  if (sp.length === 0) return `<div style="color:var(--text-faint);text-align:center;padding:30px;font-size:.85rem">No rival approaches currently. Rival villages may approach your high-performing shinobi at any time.</div>`
  return sp.map(pressure => {
    const s = (G.shinobi || []).find(x => x.id === pressure.shinobiId)
    if (!s) return ''
    return `<div style="background:var(--surface);border:1px solid var(--orange);border-radius:6px;padding:14px;margin-bottom:10px">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        <span style="font-size:1.1rem">📨</span>
        <div style="flex:1">
          <div style="color:var(--gold-hi);font-weight:bold">${pressure.villageName} has approached ${sn(s)}</div>
          <div style="font-size:.75rem;color:var(--text-dim)">Offer: <strong style="color:var(--gold)">${fmt(pressure.offerRyo)} ryo</strong> · Expires Y${pressure.expiresYear}M${pressure.expiresMonth}</div>
        </div>
        <div style="font-size:.75rem;color:var(--text-mid);text-align:right">
      ...
      <div style="display:flex;gap:6px">
        <button onclick="sellPressureBlock('${pressure.shinobiId}')" style="flex:1;background:var(--surface-2);border:1px solid var(--blue);color:var(--blue);border-radius:4px;padding:6px;cursor:pointer;font-size:.75rem">
          🚫 Block approach<br><span style="font-size:.68rem;color:var(--text-faint)">−5 rel with ${pressure.villageName}</span>
        </button>
        <button onclick="sellPressureAccept('${pressure.shinobiId}')" style="flex:1;background:var(--green-bg);border:1px solid var(--green-hi);color:var(--green);border-radius:4px;padding:6px;cursor:pointer;font-size:.75rem">
          💰 Accept offer<br><span style="font-size:.68rem;color:var(--text-faint)">+${fmt(pressure.offerRyo)} ryo</span>
        </button>
        <button onclick="sellPressureLetDecide('${pressure.shinobiId}')" style="flex:1;background:var(--red-bg);border:1px solid var(--red);color:var(--red-soft);border-radius:4px;padding:6px;cursor:pointer;font-size:.75rem">
          🤔 Let them decide<br><span style="font-size:.68rem;color:var(--text-faint)">${t("transfers.loyaltyCheck")}</span>
        </button>
      </div>
    </div>`
  }).join('')
...
function renderLoans(tm) {
  ...
  const lIn = tm.loanIn || []
  return `
    <h3 style="color:var(--text-mid);font-size:.82rem;text-transform:uppercase;letter-spacing:.08em;margin:0 0 8px">Loans Out (${lOut.length})</h3>
    ${lOut.length === 0
      ...
      : lOut.map(lo => {
          ...
          return s ? `<div style="background:var(--surface);border:1px solid var(--border);border-radius:5px;padding:10px;margin-bottom:6px;display:flex;align-items:center;gap:10px;font-size:.8rem">
            ...
            <button onclick="recallLoan('${lo.shinobiId}')" style="background:var(--red-bg);border:1px solid var(--red);color:var(--red-soft);border-radius:4px;padding:3px 8px;cursor:pointer;font-size:.72rem">${t("transfers.recallEarly")}</button>
          </div>` : ''
        }).join('')
    }
    <h3 style="color:var(--text-mid);font-size:.82rem;text-transform:uppercase;letter-spacing:.08em;margin:12px 0 8px">Loans In (${lIn.length})</h3>
    ${lIn.length === 0
    ...
    <div style="background:var(--sunken);border:1px solid var(--border);border-radius:6px;padding:12px;margin-top:10px">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        ...
        <select id="loan-out-dur" style="background:var(--surface);color:var(--gold-hi);border:1px solid var(--text-faint);border-radius:4px;padding:4px 8px;font-size:.78rem">
          <option value="3">3 months</option>
          <option value="6">6 months</option>
        </select>
        <button onclick="sendLoan()" style="background:var(--green-bg);border:1px solid var(--green-hi);color:var(--green);border-radius:4px;padding:5px 12px;cursor:pointer;font-size:.78rem">${t("transfers.sendOnLoan")}</button>
      </div>
      <div style="font-size:.7rem;color:var(--text-faint);margin-top:4px">Monthly fee earned: salary × 1.5. Commitment decays slightly while on loan.</div>
    </div>
  `
}
...
function renderBingo(judgeLevel) {
  ...
  const promCost = 3000
  return `
    <div style="font-size:.8rem;color:var(--text-mid);margin-bottom:12px">Shinobi in the Bingo Book attract rival assassins — but also generate prestige. Suppress entries to reduce risk; promote to maximize legend gain.</div>
    ${listed.length === 0
      ? '<div style="color:var(--text-faint);text-align:center;padding:30px">No shinobi listed yet. S-rank shinobi and those with 3+ S-rank wins are auto-listed.</div>'
      : `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px">
          ${listed.map(s => {
            const tier = BINGO_TIERS[Math.min(s.bingoBookPresence, BINGO_TIERS.length - 1)]
            return `<div style="background:var(--surface);border:1px solid ${tier.color}44;border-top:2px solid ${tier.color};border-radius:6px;padding:12px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                <span style="color:${tier.color};font-size:1.3rem">${tier.icon}</span>
                <div>
                  <div style="color:var(--gold-hi);font-weight:bold">${sn(s)}</div>
                  <div style="font-size:.72rem;color:${tier.color}">${tier.n}</div>
                </div>
              </div>
              <div style="font-size:.75rem;color:var(--text-dim);margin-bottom:8px">
                Assassination risk: <span style="color:var(--red)">${Math.round(tier.assasRisk * 100)}%/mo</span>
                · Prestige: <span style="color:var(--gold)">+${tier.prestigeBonus}/mo</span>
              </div>
              ${s.bingoBookSuppressed ? '<div style="font-size:.72rem;color:var(--green);margin-bottom:6px">✓ Suppressed — attacks paused</div>' : ''}
              <div style="display:flex;gap:5px;flex-wrap:wrap">
                ${!s.bingoBookSuppressed
                  ? `<button onclick="bingoSuppress('${s.id}')" style="background:var(--surface-2);border:1px solid var(--blue);color:var(--blue);border-radius:4px;padding:4px 8px;cursor:pointer;font-size:.72rem">🛡 Suppress (${fmt(suppCost)})</button>`
                  : ''}
                ${s.bingoBookPresence < 3
                  ? `<button onclick="bingoPromote('${s.id}')" style="background:#2e1a00;border:1px solid #a64;color:var(--orange);border-radius:4px;padding:4px 8px;cursor:pointer;font-size:.72rem">⬆ Promote (${fmt(promCost)})</button>`
                  : ''}
              </div>
            </div>`
          }).join('')}
...
function renderOffers(tm) {
  ...
  if (offers.length === 0) return `<div style="color:var(--text-faint);text-align:center;padding:30px;font-size:.85rem">No offer history. Negotiate with village-listed shinobi to create offers.</div>`
  return `<div>
    ${offers.slice().reverse().map(o => {
      ...
      return `<div style="background:var(--surface);border:1px solid var(--border);border-radius:5px;padding:10px;margin-bottom:6px;display:flex;align-items:center;gap:10px;font-size:.8rem">
        <div style="flex:1">
          ...
          ${o.counterAmount ? ` <span style="color:var(--orange)">· counter: ${fmt(o.counterAmount)}</span>` : ''}
        </div>
        <span style="color:${statusColor};font-size:.75rem">${o.status.toUpperCase()}</span>
        ${o.status === 'countered'
          ? `<button onclick="acceptCounter('${o.id}')" style="background:var(--green-bg);border:1px solid var(--green-hi);color:var(--green);border-radius:4px;padding:3px 8px;cursor:pointer;font-size:.72rem">${t("transfers.acceptCounter")}</button>`
          : ''}
      </div>`
    }).join('')}
  </div>`
...
function renderHistory(tm) {
  ...
  if (deals.length === 0) return `<div style="color:var(--text-faint);text-align:center;padding:30px;font-size:.85rem">${t("transfers.noCompleted")}</div>`
  return `<div>${deals.slice().reverse().map(d => {
    ...
    return `<div style="background:var(--surface);border:1px solid var(--border);border-radius:5px;padding:9px 12px;margin-bottom:5px;display:flex;align-items:center;gap:10px;font-size:.8rem">
      ...
      ${d.fee ? `<div style="color:var(--gold);font-size:.78rem">${fmt(d.fee)} ryo</div>` : ''}
    </div>`
  }).join('')}</div>`
}
