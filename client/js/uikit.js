...
export function hvMeterHtml({ label = '', value = 0, max = 100, color = 'var(--accent)', text, title = '', small = false, bare = false, style = '' }) {
  const pct = Math.max(0, Math.min(100, max ? (value / max) * 100 : 0))
  // bare: only the track + fill, for sites that lay out their own label and figure.
  if (bare) return `<span class="hv-meter-bar"${style ? ` style="${style}"` : ''}${title ? ` title="${title}"` : ''}><span class="hv-meter-fill" style="width:${pct.toFixed(1)}%;background:${color}"></span></span>`
