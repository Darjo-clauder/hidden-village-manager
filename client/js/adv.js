...
export function adv() {
  ...
  G.shinobi.forEach(s => {
    ...
    if (s.ri < 4 && pw >= thresh[s.ri + 1] && s.months >= (s.ri + 1) * 12 && s.status === 'available') { s.ri++; s.salary = 500 + s.ri * 400; aL(sn(s) + ' promoted to ' + ['Genin','Chunin','Jonin','ANBU','S-Rank'][s.ri] + '!', 'good') }
  })
